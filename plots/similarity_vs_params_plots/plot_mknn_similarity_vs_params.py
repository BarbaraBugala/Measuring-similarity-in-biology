...
# mKNN FILES
# -----------------------------
SIM_FILE = "mknn_scores.csv"
BASELINE_FILE = "mknn_scores_baseline.csv"
        ...
        for j, emb in enumerate(emb_methods):
            ...
            for dataset_path in DATASETS:
                ...
                for model in ALL_MODELS:
                    ...
                    df = pivot_mknn(df, k_value)
                    base_df = pivot_mknn(base_df, k_value)
            ...
            if j == 0:
                ax.set_ylabel(f"mKNN Similarity (k={k_value})")

    handles, labels = axes[0][0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="lower center", ncol=3,
               title="Solid lines: dataset, dotted: baseline")

    plt.suptitle(f"Mutual kNN Similarity Across Datasets (k={k_value})", y=0.98)
    plt.tight_layout(rect=[0, 0.04, 1, 0.98])
    plt.savefig(PLOTS_ROOT / "similarity_vs_params_plots" / f"mknn_grid_k{k_value}.png")
    plt.close()
