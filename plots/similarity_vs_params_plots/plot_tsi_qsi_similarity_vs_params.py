...
(PLOTS_ROOT / "similarity_vs_params_plots").mkdir(exist_ok=True, parents=True)
...
# FILE NAMES
# -----------------------------
SIM_FILE = "qsi_tsi_scores.csv"
BASELINE_FILE = "qsi_tsi_scores_baseline.csv"
...
def pivot_metric(df, metric_col):
    """Pivot CSV to sequence_method x embedding matrix for a given metric (qsi or tsi)."""
    df = df.copy()
    emb_col = "embedding_method" if "embedding_method" in df.columns else "esm_method"
    df["embedding"] = df[emb_col].apply(get_embedding)
...
def plot_metric_grid_colored_baseline(metric_name):
        ...
        for j, emb in enumerate(emb_methods):
            ...
            for dataset_path in DATASETS:
                ...
                for model in ALL_MODELS:
                    ...
                    df = pivot_metric(df, metric_name)
                    base_df = pivot_metric(base_df, metric_name)
            ...
            if j == 0:
                ax.set_ylabel(f"{metric_name.upper()} Similarity")
    ...
    plt.suptitle(f"{metric_name.upper()} Similarity Across Datasets", y=0.98)
    plt.tight_layout(rect=[0, 0.04, 1, 0.98])
    plt.savefig(PLOTS_ROOT / "similarity_vs_params_plots" / f"{metric_name}_grid.png")
    plt.close()
...
if __name__ == "__main__":
    plot_metric_grid_colored_baseline("qsi")
    plot_metric_grid_colored_baseline("tsi")